...
class HomeActivity : AppCompatActivity() {
    ...
    val linkedInUri = "https://www.linkedin.com/company/msp-tech-club-asu/"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.bottomNavigation.setOnNavigationItemSelectedListener() { item ->
            when(item.itemId) {
                ...
                R.id.page_4 -> {
                    loadEventsFragment()
                    Toast.makeText(this , "hello from 4" , Toast.LENGTH_LONG).show()
                    true
                }
    ...
    private fun loadEventsFragment() {
        val transaction = supportFragmentManager.beginTransaction()
        transaction.replace(R.id.fragment_container_view, Events())
        transaction.addToBackStack(null)
        transaction.commit()
    }
}